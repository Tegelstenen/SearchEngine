   ...
   /* ----------------------------------------------- */
    ...
    void displayResults( int maxResultsToDisplay, double elapsedTime ) {
        ...
        for ( i=0; i<results.size() && i<maxResultsToDisplay; i++ ) {
            ...
            // ############################
            String dddd =  i + " " + displayableFileName( engine.index.docNames.get( results.get(i).docID ));
            try (PrintWriter out = new PrintWriter(new FileWriter("error.txt", true))) { // Changed 'true' to 'false'
                out.println(dddd);
            } catch (IOException e) {
                e.printStackTrace();
            }
