                ...
                if ( f.isDirectory() ) {
                    ...
                    try {
                        ...
                        try (DataOutputStream dos = new DataOutputStream(
                            new BufferedOutputStream(new FileOutputStream("term_freqs.bin", true)))) {
                            
                            dos.writeInt(docID);
                            dos.writeInt(termFreq.size());
                            for (Map.Entry<String, Integer> entry : termFreq.entrySet()) {
                                // Write the token and its frequency in binary form
                                dos.writeUTF(entry.getKey());
                                dos.writeInt(entry.getValue());
                            }
                            
                        } catch (IOException e) {
                            System.err.println("Error writing to term_freqs.bin: " + e.getMessage());
                        }
